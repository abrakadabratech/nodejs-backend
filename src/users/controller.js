...
async function getGiverChatProductList(req, res) {
  ...
  try {
    ...
    if (numericPageNumber > 1) {
      const startAtDocument = await db.collection('products')
        .where('userId', '==', userId)
    ...
    const productsWithChatCounts = await Promise.all(
      productsSnapshot.docs.map(async (doc) => {
        ...
        const chatCountSnapshot = await db.collection('activeChats')
...
async function blockUserChat(req, res) {
  ...
  try {
    // Check if the block action already exists
    const existingBlockSnapshot = await db
      .collection("blocks")
    ...
    if (!existingBlockSnapshot.empty) {
      return res.status(409).send("Block action already exists.");
    }

    // Add the block action to the database
...
async function unBlockUserChat(req, res) {
  ...
  try {
    // Query the database for the block action
    ...
    // Check if the block action exists
    if (blockSnapshot.empty) {
      return res.status(404).send({ message: "Block action not found." });
    }

    // Assuming there's only one document for each unique block action
    // Delete the document representing the block action
    await db.collection("blocks").doc(blockSnapshot.docs[0].id).delete();
...
async function getMyBlockedUsersList(req, res) {
  const blockerId = res.locals.uid;
  const pageSize = parseInt(req.query.pageSize, 10) || 10; // Default page size to 10
  const pageNumber = parseInt(req.query.pageNumber, 10) || 1; // Default page number to 1

  try {
    let query = db
      .collection("blocks")
      .where("blocker", "==", blockerId)
      .orderBy("createdAt") // Assuming you have a field `createdAt` for ordering
      .limit(pageSize);

    // If page number is greater than 1, find the starting point for the query
    if (pageNumber > 1) {
    ...
    const blocksSnapshot = await query.get();

    if (blocksSnapshot.empty) {
      return res.status(404).send("No blocks found.");
    }

    const blockedIds = blocksSnapshot.docs.map((doc) => doc.data().blocked);
    const blockedUsersPromises = blockedIds.map(async (userId) => {
      const userDoc = await db.collection("users").doc(userId).get();
      if (!userDoc.exists) {
        return null;
      }
      const user = userDoc.data();
      return { uid: userId, name: user.name, user_avatar: user.user_avatar };
    });
    ...
    console.error("Error fetching blocked users with pagination:", error);
